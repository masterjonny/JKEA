...
public abstract class CommandLineUtility {
	...
	/**
	 * Returns the options made available by this command line utility. The base
	 * implementation automatically provides the {@code -h,--help} option.
	 ...
	 *            or {@code null} to use the default Java command line string
	 */
	...
	private void showHelp() {
		...
		helpFormatter.printHelp(getCommandString(),
				Localization.getString(getClass(), "description"), options,
				Localization.getString(CommandLineUtility.class, "footer"),
				true);
	}

	/**
	 * Starts this command line utility with the command line arguments provided
	 * by the {@code main} method. The command line arguments are parsed into a
	 * {@link CommandLine} object and the {@code run} method is invoked.
	 * <p>
	 * At the start of this method, a specialized uncaught exception handler is
	 * registered with the calling thread. This exception handler is responsible
	 * for formatting and displaying any errors on the command line. Note that
	 * this exception handler is not removed at the end of this method; its
	 * removal is not necessary when this utility is invoked on the command
	 * line.
	 *
	 * @param args
	 *            the command line arguments
