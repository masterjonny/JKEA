...
public class PRNGTest {
	 ...
	 * @param statistics
	 *            the captures statistics of a sampled distribution
	 */
	...
	/**
	 * Tests if the {@code nextInt} method produces uniformly-distributed values
	 * using a histogram.
	 */
	@Test
	 ...
	 *
	 * @param n
	 *            the number of {@code true} values
	 * @param array
	 *            the permutation
	 */
	...
	public void testShuffleArray(Class<?> type, int size) throws Exception {
		...
		final DescriptiveStatistics[] statistics = new DescriptiveStatistics[size];
		...
		for (int i = 0; i < 50000; i++) {
			final Method method = PRNG.class.getMethod("shuffle", type);
			method.invoke(null, array);

			final Integer[] integerArray = new Integer[size];

			for (int j = 0; j < size; j++) {
				final int value = ((Number) Array.get(array, j)).intValue();
	...
	public void testShuffleBooleanArray() {
		final int K = 4;
		final int P = 2 * K;
		final boolean[] array = new boolean[P];
		final DescriptiveStatistics[] statistics = new DescriptiveStatistics[P];
	...
	/**
	 * Tests if the {@code shuffle} method produces valid permutations of a byte
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty byte
	 * array.
	 */
	@Test
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty double
	 * array.
	 */
	@Test
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty float
	 * array.
	 */
	@Test
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty integer
	 * array.
	 */
	@Test
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty long
	 * array.
	 */
	@Test
	...
	/**
	 * Tests if the {@code shuffle} method correctly handles an empty short
	 * array.
	 */
	@Test
	...
	public void testShuffleList() {
		...
		final List<DescriptiveStatistics> statistics = new ArrayList<DescriptiveStatistics>();
	...
	/**
	 * Tests if the {@code shuffle} method produces valid permutations of a long
	 * array, and that the distribution of the values for each index are
	 * approximately uniform.
	 *
	 * @throws Exception
	 ...
	 *
	 * @param min
	 *            the minimum bounds of the uniform distribution
	 * @param max
	 *            the maximum bounds of the uniform distribution
	 * @param statistics
	 *            the captures statistics of a sampled distribution
	 */
	private void testUniformDistribution(double min, double max,
		...
		Assert.assertEquals((min + max) / 2.0, statistics.getMean(),
				TestThresholds.STATISTICS_EPS);
		Assert.assertEquals(Math.pow(max - min, 2.0) / 12.0,
				statistics.getVariance(), TestThresholds.STATISTICS_EPS);
		Assert.assertEquals(0.0, statistics.getSkewness(),
				TestThresholds.STATISTICS_EPS);
	...
	private void testUniformDistribution(int min, int max,
		...
		Assert.assertEquals(0.0, statistics.getSkewness(),
				TestThresholds.STATISTICS_EPS);
		Assert.assertEquals(-(6.0 * (nn + 1)) / (5.0 * (nn - 1)),
				statistics.getKurtosis(), TestThresholds.STATISTICS_EPS);
		Assert.assertEquals(min, statistics.getMin(),
				TestThresholds.STATISTICS_EPS);
		Assert.assertEquals(max, statistics.getMax(),
				TestThresholds.STATISTICS_EPS);
